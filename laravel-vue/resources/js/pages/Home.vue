...
export default {
    ...
    methods: {
        ...
        async fetchTipeData() {
            ...
            try {
                const response = await fetch(
                    `${API_BASE_URL}/api/v1/guest/akademik/tipe-tes-masuk`
                );
                if (response.status === 404)
                    throw new Error("Data belum tersedia (404)");
                if (!response.ok)
                ...
                this.tipeChartData = {
                    labels: rawData.map(
                        (item) => item.tipe || item.nama || item.jenis
                    ),
                    datasets: [
                        {
                            label: "Jumlah Mahasiswa",
...
@media (max-width: 768px) {
    ...
    .charts-grid {
        grid-template-columns: 1fr; /* 1 Kolom Chart di mobile */
    }
    .span-full,
    .span-col-2 {
        grid-column: span 1; /* Reset span di mobile */
    }
    .span-row-2 {
        grid-row: span 1; /* Reset row span di mobile */
    }
    .chart-tall {
        min-height: 300px; /* Kurangi height di mobile */
    }
    .welcome-banner {
        padding: 1.5rem;
    }
